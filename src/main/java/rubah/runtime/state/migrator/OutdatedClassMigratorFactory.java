...
public class OutdatedClassMigratorFactory extends DefaultObjectMigratorFactory {
	private Version v1;
	...
	public OutdatedClassMigratorFactory(
	...
	private ConversionInfo getConversionInfo(Class<?> c) throws ClassNotFoundException, NoSuchMethodException, SecurityException {
		...
		if (ret == null) {
			...
			Class<?> redefinedClass = Class.forName(v1.getUpdatableName(c1.getFqn()), true, Rubah.getLoader());
			ret.redefinedClassToken = UnsafeUtils.getInstance().getClassToken(redefinedClass);
	...
	protected class UpdatableObjectMigrator extends DefaultObjectMigrator {
		...
		protected Object doMigrate(Object pre) {
			try {
				...
				UnsafeUtils.getInstance().changeClass(pre, info.redefinedClassToken);
